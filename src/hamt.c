...
#define BITS 64
#define K 6
#define MAX_DEPTH (BITS / K)

ALWAYS_INLINE static inline size_t popcount(uint64_t x)
{
	return __builtin_popcount(x);
}

ALWAYS_INLINE static inline size_t get_index(uint64_t bitmap,
										 ...
										 void *val, uint64_t hash, size_t shift)
{
	...
	if (old_hash == hash) {
		/* Complete collision */
		if (UNLIKELY(!h->cmp(old_node->kv.key, key))) {
...
/*
 * NOTE: This algorithm is quite slow because removing and keeping the HAMT in its most compact form is tedious.
 * A bunch of small optimizations are applied here to try to make this as fast as possible, using compiler attributes and caching intermediate values.
*/
void hamt_remove(struct hamt *h, void *key)
{
	struct hamt_node *ancestors[MAX_DEPTH];
	int popcounts[MAX_DEPTH];
	...
	popcounts[ancestors_count++] = popcount(node->branch.bitmap);
	...
	while (true) {
		if (IS_VALUE(node->kv.value)) {
			...
			if (UNLIKELY(!h->cmp(node->kv.key, key))) {
				struct hamt_node *prev = node;
				for (struct hamt_node *curr = node->kv.next; curr;
					 prev = curr, curr = curr->kv.next) {
					if (h->cmp(curr->kv.key, key)) {
						prev->kv.next = curr->kv.next;
						h->alloc.free(curr, sizeof(struct hamt_node));
						return;
					}
				}
			...
			/* Replace the node with its `next` element in the chain */
			if (node->kv.next) {
		...
		if (LIKELY(node->branch.bitmap & (1UL << idx))) {
			ancestors[ancestors_count] = node;
			popcounts[ancestors_count++] = popcount(node->branch.bitmap);
	...
	size_t old_size = popcounts[ancestors_count - 1];

	/* There is only one other leaf */
	if (old_size == 2) {
		size_t child_index = get_index(parent->branch.bitmap, idx);
		struct hamt_node other_node = parent->branch.children[!child_index];
		...
		if (LIKELY(!IS_VALUE(other_node.kv.value)) || prune_index == 0) {
			/* Shrink the table */
			parent->branch.bitmap &= ~(1UL << idx);

			shrink_table_to_fit(h, parent, old_size - 1, old_size, child_index);
			return;
		}
